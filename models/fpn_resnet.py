...
class PoseResNet(nn.Module):
    ...
    def __init__(self, block, layers, heads, head_conv, **kwargs):
        ...
        self.kfpn_features = []
        self.fpn_outputs = {}  # Store FPN outputs for each head
        self.kfpn_weights = {}  # Store KFPN attention weights
        self.backbone_features = {}  # Store backbone features
    ...
    def forward(self, x):
        ...
        # Store backbone features for visualization
        self.backbone_features = {
            'layer1': out_layer1.detach().clone(),
            'layer2': out_layer2.detach().clone(),
            'layer3': out_layer3.detach().clone(),
            'layer4': out_layer4.detach().clone()
        }
        ...
        # Store KFPN features for visualization
        self.kfpn_features = [
            up_level2.detach().clone(),
            up_level3.detach().clone(), 
            up_level4.detach().clone()
        ]

        ret = {}
        for head in self.heads:
            temp_outs = []
            fpn_level_outs = []  # Store individual FPN level outputs
            ...
            for fpn_idx, fdn_input in enumerate([up_level2, up_level3, up_level4]):
                ...
                fpn_level_outs.append(fpn_out.detach().clone())
            
            # Store FPN outputs for each head and level
            self.fpn_outputs[head] = fpn_level_outs
            
            # Take the softmax in the keypoint feature pyramid network
            final_out, attention_weights = self.apply_kfpn(temp_outs)
            
            # Store attention weights for visualization
            self.kfpn_weights[head] = attention_weights.detach().clone()
    ...
    def apply_kfpn(self, outs):
        outs_stacked = torch.cat([out.unsqueeze(-1) for out in outs], dim=-1)
        softmax_outs = F.softmax(outs_stacked, dim=-1)
        ret_outs = (outs_stacked * softmax_outs).sum(dim=-1)
        
        # Return both the final output and attention weights for visualization
        return ret_outs, softmax_outs

    def get_visualization_data(self):
        """Return all stored features for visualization"""
        return {
            'backbone_features': self.backbone_features,
            'kfpn_features': self.kfpn_features,
            'fpn_outputs': self.fpn_outputs,
            'kfpn_weights': self.kfpn_weights
        }
