...
def post_processing(detections, num_classes=3, down_ratio=4, peak_thresh=0.2):
    ...
    print(f"Input detections shape: {detections.shape}")
    ...
    for i in range(detections.shape[0]):
        ...
        print(f"Processing batch {i}, number of detections: {len(classes)}")
        for j in range(num_classes):
            ...
            ## my changes
            print("top_preds=",top_preds[j])
            print("lenght of top_preds[j]=",len(top_preds[j]))
            print(f"Class {j} has {len(top_preds[j])} detections.")
            ## my changes end
            # Filter by peak_thresh
            if len(top_preds[j]) > 0:
                keep_inds = (top_preds[j][:, 0] > peak_thresh)
                print(f"Class {j} after filtering has {np.sum(keep_inds)} detections.")
                top_preds[j] = top_preds[j][keep_inds]
            ## my changes
            if len(top_preds[j]) == 0:
                print(f"Warning: No predictions for class {j} in batch {i}.")
