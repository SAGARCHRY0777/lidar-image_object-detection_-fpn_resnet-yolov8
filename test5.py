...
def parse_test_configs():
    ...
    parser.add_argument('--fusion_iou_threshold', type=float, default=0.8, help='IoU threshold for fusing detections from different models')
...
def fuse_overlapping_detections(yolov8_detections, sfa3d_detections, fusion_iou_threshold):
    """
    Fuses overlapping detections from YOLOv8 and SFA3D using a weighted average.
    Prioritizes higher confidence scores in the weighted average.
    """
    fused_detections = []
    
    # Keep track of which SFA3D detections have been fused
    ...
    for yolo_det in yolov8_detections:
        ...
        for i, sfa3d_det in enumerate(sfa3d_detections):
            if sfa3d_matched[i]: # Skip already matched SFA3D detections
                continue
        ...
        if best_match_sfa3d_idx != -1:
            # Fuse with a weighted average
            sfa3d_det = sfa3d_detections[best_match_sfa3d_idx]
            sfa3d_box = sfa3d_det['box']
            sfa3d_conf = sfa3d_det['confidence']
            
            # Weights based on confidence
            total_conf = yolo_conf + sfa3d_conf
            
            if total_conf == 0: # Avoid division by zero if both confidences are 0
                weight_yolo = 0.5
                weight_sfa3d = 0.5
            else:
                weight_yolo = yolo_conf / total_conf
                weight_sfa3d = sfa3d_conf / total_conf
            
            fused_x = int(weight_yolo * yolo_box[0] + weight_sfa3d * sfa3d_box[0])
            fused_y = int(weight_yolo * yolo_box[1] + weight_sfa3d * sfa3d_box[1])
            fused_w = int(weight_yolo * yolo_box[2] + weight_sfa3d * sfa3d_box[2])
            fused_h = int(weight_yolo * yolo_box[3] + weight_sfa3d * sfa3d_box[3])
            
            fused_confidence = max(yolo_conf, sfa3d_conf) # Take the max confidence
            
            fused_detections.append({
                'box': [fused_x, fused_y, fused_w, fused_h],
                'confidence': fused_confidence,
                'class_id': yolo_det['class_id'], # Keep YOLOv8's class ID, assuming it's more general
                'class_name': yolo_det['class_name'],
                'model': 'Fused (YOLOv8 + SFA3D)', # Indicate it's a fused detection
...
def create_fused_detections(yolov8_data, sfa3d_data, confidence_threshold, fusion_iou_threshold):
    """
    Creates an initial list of detections from both models and then fuses
    overlapping ones.
    """
    yolov8_boxes, yolov8_confidences, yolov8_class_ids, yolov8_class_names = yolov8_data
    sfa3d_boxes, sfa3d_confidences = sfa3d_data
    
    initial_yolov8_detections = []
    for i, (box, conf, class_id) in enumerate(zip(yolov8_boxes, yolov8_confidences, yolov8_class_ids)):
        if conf >= confidence_threshold:
            initial_yolov8_detections.append({
                'box': box,
                'confidence': conf,
                'class_id': class_id,
    ...
    initial_sfa3d_detections = []
    for i, (box, conf) in enumerate(zip(sfa3d_boxes, sfa3d_confidences)):
        if conf >= confidence_threshold:
            initial_sfa3d_detections.append({
                'box': box,
                'confidence': conf,
                'class_id': 0,  # Assuming car class for SFA3D
                'class_name': 'car',
                'model': 'SFA3D',
                'color': (255, 0, 0)  # Blue for SFA3D
            })
            
    fused_detections_list = fuse_overlapping_detections(
        initial_yolov8_detections, initial_sfa3d_detections, fusion_iou_threshold
    )
    
    return fused_detections_list
...
def create_detection_summary(fused_detections_before_nms, fused_detections_after_nms):
    ...
    fused_before = sum(1 for d in fused_detections_before_nms if d['model'] == 'Fused (YOLOv8 + SFA3D)')
    ...
    fused_after = sum(1 for d in fused_detections_after_nms if d['model'] == 'Fused (YOLOv8 + SFA3D)')
    ...
    cv2.putText(summary_img, f"YOLOv8: {yolo_before}", (50, 110), font, 0.6, (0, 255, 255), 2)
    cv2.putText(summary_img, f"SFA3D: {sfa_before}", (50, 140), font, 0.6, (255, 0, 0), 2)
    cv2.putText(summary_img, f"Fused: {fused_before}", (50, 170), font, 0.6, (0, 255, 0), 2)
    cv2.putText(summary_img, f"Total: {len(fused_detections_before_nms)}", (50, 200), font, 0.6, (255, 255, 255), 2)
    
    cv2.putText(summary_img, "After NMS:", (300, 80), font, 0.6, (255, 255, 255), 2)
    cv2.putText(summary_img, f"YOLOv8: {yolo_after}", (300, 110), font, 0.6, (0, 255, 255), 2)
    cv2.putText(summary_img, f"SFA3D: {sfa_after}", (300, 140), font, 0.6, (255, 0, 0), 2)
    cv2.putText(summary_img, f"Fused: {fused_after}", (300, 170), font, 0.6, (0, 255, 0), 2)
    cv2.putText(summary_img, f"Total: {len(fused_detections_after_nms)}", (300, 200), font, 0.6, (255, 255, 255), 2)
    
    # Legend
    cv2.putText(summary_img, "Legend:", (50, 250), font, 0.6, (255, 255, 255), 2)
    cv2.rectangle(summary_img, (50, 260), (70, 280), (0, 255, 255), -1)
    cv2.putText(summary_img, "YOLOv8", (80, 275), font, 0.5, (255, 255, 255), 1)
    cv2.rectangle(summary_img, (200, 260), (220, 280), (255, 0, 0), -1)
    cv2.putText(summary_img, "SFA3D", (230, 275), font, 0.5, (255, 255, 255), 1)
    cv2.rectangle(summary_img, (350, 260), (370, 280), (0, 255, 0), -1)
    cv2.putText(summary_img, "Fused", (380, 275), font, 0.5, (255, 255, 255), 1)
...
if __name__ == '__main__':
    ...
    output_dir = "D:\\spa\\SFA3D\\sfa\\fused_detection_results_weighted_fusion"
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(test_dataloader):
            ...
            # === Weighted Fusion and NMS ===
            # Create fused detections with weighted combination for overlapping boxes
            ...
            print(f"Total detections after weighted fusion (before NMS): {len(fused_detections_before_nms)}")
            
            # Apply NMS on the combined set of detections
            ...
            # Add labels
            cv2.putText(combined_top, "Before Final NMS (Weighted Fusion Applied)", (50, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
            cv2.putText(combined_top, "After Final NMS", (690, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
            ...
            try:
                ...
                print("tkinter not found, using default window size.")
            ...
            print(f'\nSFA3D Processing time: {(t2 - t1) * 1000:.1f}ms')
            print(f'Detection Summary:')
            print(f'  - Before Fusion & NMS:')
            print(f'    - YOLOv8 Raw: {len(yolov8_boxes)}')
            print(f'    - SFA3D Raw: {len(sfa3d_boxes_2d)}')
            print(f'  - After Weighted Fusion (Before Final NMS): {len(fused_detections_before_nms)}')
            print(f'  - After Final NMS: {len(fused_detections_after_nms)}')
            print('\n[INFO] Press n for next sample | Press Esc to quit\n')
            
            key = cv2.waitKey(0) & 0xFF
            if key == 27:  # Esc key
                break
            elif key == ord('n'): # 'n' for next sample
                continue # loop to next batch
            elif key == ord('c'): # 'c' to close window and continue
